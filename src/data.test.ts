...
describe('容量 domain', () => {
  ...
  it('規劃範圍內沒有容量時，將剩餘工時放在最後一天並保留超載結果', () => {
    ...
    expect(value.allocations).toMatchObject([
      { date: '2026-01-02', allocatedHours: 10, source: 'automatic' },
    ]);
  });
  ...
  it('日層級增加工時會先消耗 Pending，再從自動分配尾端挪出工時', () => {
    ...
    expect(result.allocations.find(item => item.date === '2026-01-03')).toMatchObject({
      source: 'manual',
      allocatedHours: 1,
      locked: true,
    });
  ...
  it('日層級減少工時會保留手動零值，並把工時補回自動尾端', () => {
    ...
    expect(result.allocations.find(item => item.date === '2026-01-02')).toMatchObject({
      source: 'manual',
      allocatedHours: 1,
      locked: true,
    });
    expect(result.allocations.find(item => item.date === '2026-01-01')).toMatchObject({
      source: 'automatic',
      allocatedHours: 7,
    });
    expect(getTaskPendingHours(value, result.allocations)).toBe(0);
  ...
  it('點擊沒有工時的日期也會建立手動零值鎖定', () => {
    ...
    expect(result.allocations).toMatchObject([
      { date: '2026-01-01', allocatedHours: 0, source: 'manual', locked: true },
    ]);
  });
  ...
  it('日期範圍不能排除 Manual Allocation', () => {
    ...
    expect(() => recalculateAutomaticAllocations(value, [manual], [])).toThrow(
      '日期範圍不可排除人工分配日期',
    );
  });
  ...
  it('調整一個 Task 的 Allocation 不會改動其他 Task', () => {
    ...
    expect(
      result.allocations
        .filter(item => item.taskId === 'task-a')
        .reduce((sum, item) => sum + item.allocatedHours, 0),
    ).toBe(9);
  });
});
...
describe('資料工具', () => {
  ...
  it('週／月拖曳會依目前時間軸層級移動', () => {
    ...
    expect(applyTaskDrag(value, 'move', 1, 'week')).toMatchObject({
      start: '2026-02-07',
      end: '2026-02-22',
    });
    expect(applyTaskDrag(value, 'move', 1, 'month')).toMatchObject({
      start: '2026-02-28',
      end: '2026-03-15',
    });
  });
