...
describe('語意時間軸',()=>{
 ...
 it('switches semantic levels with hysteresis',()=>{
  expect(zoomTimeline({view:'day',pixelsPerDay:16.1},.99).view).toBe('week');
  expect(zoomTimeline({view:'week',pixelsPerDay:4.1},.97).view).toBe('month');
 ...
 it('keeps task positions at actual dates inside aggregated periods',()=>{
  ...
  expect(geometry?.width).toBeCloseTo(timelinePositionForDate('2026-07-19',periods,scale)-geometry!.left);
 });
});
