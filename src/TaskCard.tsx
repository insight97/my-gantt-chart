...
export default function TaskCard({task,variant,allocatedHours=0,pendingHours=0,isDragging=false,isGhost=false,onEdit,onDelete,onPointerDown}:TaskCardProps){
 ...
 return <article className={className} draggable={false} tabIndex={isGhost?undefined:0} aria-hidden={isGhost||undefined} onPointerDown={onPointerDown?handlePointerDown:undefined} onClick={handleClick} onKeyDown={handleKeyDown}>
  <div className={`task-card-info${variant==='gantt'?' task-link':''}`} draggable={false}><b>{task.name}</b><small>{variant==='backlog'?<><em className={`priority ${task.priority}`}>{priorityLabels[task.priority]}</em> · 預估 {hoursLabel(task.estimatedHours)}</>:<>{task.deadline?`截止 ${task.deadline} · `:''}{formatRange(task)}</>}</small></div>
