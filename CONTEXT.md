...
**Hierarchy**：有子項目的 Work Item 不能直接分配工時；其預估工時與 Allocation 由所有後代葉節點彙總。拖曳 Work Item 會帶著整個子樹一起移動。
...
**Backlog to Allocation Timeline**：同一份 Work Item 資料在兩個位置之間移動，不建立副本。拖曳到時間軸空白區會觸發 Automatic Scheduling；拖曳到另一個 Work Item 則依落點加入子項目或改變同層順序。

**Allocation Timeline to Backlog**：使用者可以將 Timeline 的 Work Item card 拖回 Backlog，或在 editor 明確切換狀態。兩者都是同一個移回操作：清除所有 Allocation、保留 Deadline 與其他 metadata。
...
**Allocation Timeline**：唯一保留的時間軸畫面，固定採 Allocation Adjustment 的操作語意。它保留日、週、月的容量與 Allocation 顯示、時間軸平移與縮放，以及 Backlog Task 放入時間軸的操作；不繪製 Task bar，改以淺色底顯示第一個到最後一個正 Allocation 日期的範圍，實際有工時的格子使用較深底色，日層級週末在日期標題標記、下方格子不加入週末紋理。Task Date Range metadata 顯示在 Task card 上。
